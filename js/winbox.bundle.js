(function () {
	...
	function V() {
		for (var a = w.length, b = 0, c, g; b < a; b++)
			(c = w[b]),
				...
				c.resize((g + 1) | 0, 35, !0).move((c.left + b * g) | 0, Q - c.bottom - 35, !0);
	}
	function W(a, b) {
		function c(d) {
			p(d);
		...
		function g(d) {
			...
			if (K || r)
				K && (a.width = Math.max(Math.min(a.width, P - a.x - a.right), 150)),
					r && (a.height = Math.max(Math.min(a.height, Q - a.y - a.bottom), 35)),
					a.resize();
			if (A || v)
	...
	e.fullscreen = function (a) {
		if ("undefined" === typeof a || a !== E)
			this.min && (this.resize().move(), X(this)), (E && Y()) || (this.body[I](), (E = !0));
		return this;
	};
	function Y() {
		E = !1;
